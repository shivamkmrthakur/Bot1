...
CHANNEL_ID = "-1002573368807"  # yaha apna channel id daalo (negative me)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            await update.message.reply_text(f"✅ You are a member!\nHere is your requested link: https://example.com/video/{video_id}")
        else:
...
async def joined_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        member = await context.bot.get_chat_member(CHANNEL_ID, user_id)
        if member.status in ["member", "administrator", "creator"]:
            await query.edit_message_text("✅ Thanks for joining! Now send /start 302 again to get your link.")
        else:
